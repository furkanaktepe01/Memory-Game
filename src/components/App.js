...
class App extends Component {
  ...
  laodBlockchainData = async () => {
    ...
    if (networkData) {
      ...
      const address = networkData.address; console.log(address);
      
      const token = new this.state.web3.eth.Contract(abi, address);
      this.setState({ token });
      ...
      for (let i = 0; i < balanceOf; i++) {
        let id = await token.methods.tokenOfOwnerByIndex(account, i);
